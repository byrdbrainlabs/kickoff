...
Testing is the build's business — unit, integration, UI-driven, a humble-object seam, TDD or none: all past the seam, all experimentation surface. But the spec quietly guarantees everything needed to write a **conformance suite independently of any build**: every element a ticket names is a **touch point** (locatable by role + accessible name, via the foundation's semantic-addressability commitment), every EARS criterion carries a stable ID with `supersedes` tracking the active contract through reversals, and the foundation's launch-contract and initial-state commitments give such a suite a way to start, reach, and reset the product. Nothing in this repo builds that suite — when a backlog has matured enough to deserve one, it can be authored straight from the tickets, kept as fixture alongside them, and used to score any build of the backlog — including a replay on a different harness after the original product is deleted.
...
- **Criterion IDs** — every EARS criterion in a ticket carries a stable ID (`AC-012.3`), assigned once and never renumbered. A later ticket retires an ID by name (`supersedes`) when it changes behavior, so the **active acceptance contract** — all criteria minus superseded ones — is computable from frontmatter alone.
- **Touch points** — a ticket makes something testable simply by naming it: ubiquitous language plus the foundation's semantic-addressability commitment make every named element programmatically locatable (role + accessible name). This is as far as the spec goes toward testing — no selectors, no test hooks, no constraint on how a build tests itself (see [Verifiability](#verifiability)).
...
- **Frontmatter exports** — a fleshed ticket declares in frontmatter what it `introduces` (canonical terms defined), `modifies` (behavior changed), `supersedes` (criterion IDs retired), and `depends_on` (lower-numbered tickets it requires built). Agents skim frontmatter + Scope across the backlog and read full bodies only where names overlap; schedulers compute readiness from `depends_on` alone.
